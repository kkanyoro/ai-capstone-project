...
class HealthcareDiagnosticAgent:
    ...
    def think(self):
        ...
        # Run each registered module EXCEPT the planner
        for module_name, module in self._modules.items():
            if module_name == 'Treatment Planner':
                continue # Hold the planner back for the Act phase
    ...
    def act(self, diagnosis_results: Dict) -> Dict:
        ...
        # Determine final diagnosis and urgency
        final_diagnosis = self._aggregate_diagnosis(diagnosis_results)
        urgency = self._assess_urgency(patient, avg_confidence)
        
        # NEW: Now that we have the final diagnosis, call the AI Planner
        treatment_plan = {}
        if 'Treatment Planner' in self._modules:
            self._log(f"  [Treatment Planner] Generating plan for: {final_diagnosis}")
            treatment_plan = self._modules['Treatment Planner'].create_treatment_plan(
                final_diagnosis, urgency)
        ...
        action_report = {
            ...
            'treatment_plan': treatment_plan # Attach the generated plan to the report
        }

...
class TreatmentPlanner:
    ...
    def _build_action_library(self) -> List[Dict]:
        ...
        return [
            ...
            {
                'name': 'AdministerEmergencyTreatment',
                'precond': {'PATIENT_IN_ICU'},
                'delete':  set(),
                'add':     {'TREATMENT_STARTED'},
                'cost': 2, 'duration': 'Immediate'
            },
            # Diagnostics
            {
            ...
            },
            {
                ...
                # BUG FIX: Blood results should confirm a diagnosis to allow treatment
    ...
    def analyze(self, percept) -> Dict:
        ...
        # BUG FIX: Gracefully handle planner errors so the agent doesn't crash
        if 'error' in result:
             return {
                 'summary': f"Plan failed: {result['error']}",
                 'diagnosis': 'flu',
                 'confidence': 0.0
             }
